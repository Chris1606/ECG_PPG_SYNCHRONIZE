...
class SyncUARTMonitor:
    ...
    def measure_worker(self, duration_s: int):
        ...
        try:
            ...
            with serial.Serial(port, BAUD_RATE, timeout=1.0) as ser:
                ...
                self.set_status(f"Da luu raw: {raw_file.name} | filtered: {filtered_file.name}")
        except Exception as exc:
            self.set_status(f"ERROR: {exc}")
    ...
    def save_filtered_csv(self, rows: list[SyncRow], person: str, mode: str) -> Path:
        ...
        with path.open("w", newline="", encoding="utf-8") as f:
            ...
            writer.writerow([
                "person_name", "time_ms",
                "ecg_raw", f"ecg_wavelet_{WAVELET_NAME}_level{WAVELET_LEVEL}",
    ...
    def compute_filtered(self, rows: list[SyncRow]) -> tuple[np.ndarray, np.ndarray]:
        ...
        if len(ecg_idx):
            ...
            ecg_filtered[ecg_idx] = wavelet_denoise(ecg_values)
    ...
    def plot_rows(self, rows: list[SyncRow], title: str):
        ...
        if ecg_mask.any():
            ...
            self.ax_ecg.plot(t[ecg_mask], ecg_filtered[ecg_mask], color="#d62728", linewidth=1.1, label="ECG wavelet")
        self.ax_ecg.set_ylabel("ECG raw")
